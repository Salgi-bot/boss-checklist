...
if st.session_state.analyzed:
    ...
    items = [
        (1,  "구조 성능기반설계",          "판단 유보", "건축법 시행령 제32조 + KDS 17 10 00(내진설계기준)", "구조 심의 시", f"성능기반 내진설계 적용 대상 여부 구조 전문가 확인 필요 — 50층↑ 또는 H200m↑ 특수구조 건축물 의무 (현재: {TF}층, {H}m)"),
        ...
        (4,  "단지특화디자인(색채 등)",     "가" if has_gong or has_ju or "오피스텔" in usages else "부", "경관법 제28조 + 시행령 제21조 + 국토계획법 제52조", "건축심의시", "공동주택·주상복합·오피스텔 해당 — 외관색채·입면디자인·조경 등 단지 특화 항목 심의위원회 검토"),
        (5,  "소규모 지하안전영향평가",     "가" if 10 <= D < 20 else "부",      "지하안전관리특별법 제23조", "사업승인 완료 전",    f"굴착 10m↑~20m 미만, 지반침하 발생 시 관할 지자체 즉시 통보·응급조치 의무 (현재: {D}m)"),
        (6,  "지하안전영향평가",            "가" if D >= 20 else "부",           "지하안전관리특별법 제14조", "사업승인 완료 전",    f"굴착 20m↑, 지반침하 발생 시 관할 지자체 즉시 통보·응급조치 의무 (현재: {D}m)"),
        (7,  "착공후 지하안전영향평가",     "가" if D >= 20 else "부",           "지하안전관리특별법 제20조", "공사 착공 후",        f"굴착 20m↑ 공사 착공 이후 사후 평가 제출 의무 (현재: {D}m)"),
        (8,  "현황측량(지장물조사)",        "가" if D >= 10 else "부",           "공간정보관리법 제23조제1항 + 시행령 제18조", "사업계획승인신청 전", f"지하안전영향평가(10m↑) 선행 제출 사항, 지하 지장물·매설관로 현황 확인 (현재: {D}m)"),
        (9,  "지질조사",                   "가",                                "건설기술진흥법 시행령 제74조 + 지하안전법 제43조 + KDS 11 00 00", "착공시", "전 사업 의무 — 지하안전영향평가 대상은 3공(시추·CPT·공내재하) 이상 발주 권고"),
        (10, "흙막이 설계",                "가" if BF > 0 else "부",            "건설기술진흥법 제62조제11항 + 시행령 제101조의2", "인허가시", f"지하층 굴토 깊이 2m↑ 또는 인접 건축물 기초보다 깊은 경우 가설구조물 구조안전확인 의무 (현재: 지하{BF}층, 굴착{D}m)"),
        (11, "수량산출서",                 "가" if (has_gong and HH >= 20) or ("단독주택" in usages and HH >= 20) else "부", "건축법 제23조 + 국토부 고시 건축물설계도서작성기준", "착공시", f"공동주택 20세대↑ / 단독주택 20호↑ / 도시형생활주택 30세대↑ 사업승인 신청 필수 첨부 (현재: {HH}세대)"),
        ...
        (15, "수질오염물질 총량제",         "가" if (has_gong and HH>=30) or (has_ju and HH>=30) else "부", "물환경보전법 제4조의2~제4조의4 + 환경부 오염총량기본방침", "사업계획승인신청시", f"30세대↑ 공동주택·주상복합 — 유역환경청 협의 필요, 오염부하량 할당 초과 시 사업 불가 (현재: {HH}세대)"),
        ...
        (23, "에너지소비 총량제(ECO2)",    "가" if (excl_A>=3000 and any(u in ["업무시설", "교육연구시설(연구소·도서관 제외)"] for u in usages)) or (is_public and T>=500) else "부", "녹색건축물법 제15조 + 국토부 고시 에너지절약설계기준", "사업계획승인신청시", f"업무·교육 3,000㎡↑ / 공공 500㎡↑ — 용도별 단위면적당 에너지소비량(kWh/㎡·년) 기준 이내 설계 (현재: {excl_A:,}㎡, {T:,}㎡)"),
        ...
        (32, "사전재난영향성검토",          "가" if TF>=50 or H>=200 or is_under_link else "부", "초고층재난법 제6조(검토협의) + 제7조(위원회)", "허가등을 하기 전", f"50층↑ OR 200m↑ OR 지하연계복합 — 소방·재난 통합방재계획 포함 (현재: {TF}층, {H}m)"),
        ...
        (36, "사전경관계획 심의",           "가" if L >= 300000 or T >= 200000 else "부", "경관법 제27조 + 시행령 제19조·제20조", "도시계획심의시", f"대지 30만㎡↑ OR 연면적 20만㎡↑ 개발사업 — 경관계획 수립 후 시·도 경관위원회 심의 (현재: {L:,}㎡, {T:,}㎡)"),
        ...
        (42, "지하철(철도) 영향성 검토",   "가" if 0 < rail_D <= 30 else "부",  "철도안전법 제45조(보호지구 행위제한) + 제46조(안전조치)", "착공 전",  f"철도보호지구(철도경계선 30m이내) — 노반·궤도 진동·침하 검토 후 철도운영기관 협의 필요 (현재: {rail_D}m)")
    ]
