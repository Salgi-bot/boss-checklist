...
if st.session_state.analyzed:
    ...
    c10_targets = ["다가구주택", "공동주택(아파트)", "연립주택 및 다세대주택", "오피스텔", "제1종 근린생활시설(일용품 소매점)", "제2종 근린생활시설(다중생활시설)", "문화 및 집회시설(동·식물원 제외)", "교육연구시설(연구소·도서관 제외)", "노유자시설", "수련시설"]

    # [수정 4] 42개 항목 완벽 복구
    items = [
        (1, "구조 성능기반설계", "판단 유보", "내진설계 지침", "구조 심의 시"),
        (2, "설계안전보건대장", "가" if cost >= 5000000000 else "부", "안전보건대장 고시", f"공사비 50억 이상, 기본-설계-공사 3단계 분리 명기 (현재: {const_cost}억원)"),
        ...
        (5, "소규모 지하안전영향평가", "가" if 10 <= D < 20 else "부", "지하안전법", f"굴착 10~20m 미만, 1㎡/1m 이상 침하 시 지자체 즉시 통보·응급조치 의무 (현재: {D}m)"),
        (6, "지하안전영향평가", "가" if D >= 20 else "부", "지하안전법", f"굴착 20m 이상, 1㎡/1m 이상 침하 시 지자체 즉시 통보·응급조치 의무 (현재: {D}m)"),
        ...
        (11, "수량산출서", "가" if (has_gong and HH >= 20) or ("단독주택" in usages and HH >= 20) else "부", "제출 기준", f"공동주택 20세대/단독주택 20호/도시형생활주택 30세대 이상 (현재: {HH}세대)"),
        (12, "단지내 소음예측평가", "가" if is_biz_app else "부", "주택건설기준 9조", "사업계획승인 대상 해당"),
        (13, "범죄예방 건축기준", "가" if any(u in c10_targets for u in usages) else "부", "건축법 53조의2", "500세대 미만 아파트·다가구·다세대·연립·오피스텔 등 전면 의무 적용 확대"),
        (14, "에너지절약설계기준", "가" if excl_A >= 500 or has_gong else "부", "녹색건축물법", f"비주거 500㎡이상 또는 공동주택 (현재: {excl_A:,}㎡)"),
        (15, "수질오염물질 총량제", "가" if (has_gong and HH>=30) or (has_ju and HH>=30) else "부", "오염총량방침", f"30세대 이상 공동주택 또는 주상복합 (현재: {HH}세대)"),
        (16, "녹색건축인증", "가" if ((has_gong and HH>=30) or excl_A>=500) else "부", "녹색건축물법", f"주거30세대/비주거500㎡ (현재: {HH}세대, {excl_A:,}㎡)"),
        (17, "제로에너지건축물 인증", "가" if (has_gong and HH>=30) or (is_public and T>=500) or (T>=1000 and ("업무시설" in usages or is_public)) else "부", "녹색건축물법", f"공공건축물 500㎡ 이상 / 민간·공공 공동주택 30세대 이상 전면 의무화 (현재: {HH}세대, {T:,}㎡)"),
        (18, "에너지절약형 친환경주택", "가" if has_gong and HH>=30 and is_biz_app else "부", "주택건설기준 64조", f"30세대 이상 사업승인 공동주택 (현재: {HH}세대)"),
        (19, "건강친화형 주택 건설기준", "가" if has_gong and HH>=500 else "부", "주택건설기준 65조", f"500세대 이상 공동주택, 감리자 자체평가 이행확인서 제출·가산비용 인정 연계 (현재: {HH}세대)"),
        (20, "공동주택 결로 방지 설계", "가" if has_gong and HH>=500 else "부", "주택건설기준 14조", f"500세대 이상 공동주택 (현재: {HH}세대)"),
        (21, "장수명 주택 건설인증", "가" if has_gong and HH>=1000 else "부", "주택건설기준 65조", f"1,000세대 이상 공동주택, 설계기준강도 21MPa 상향·우수등급 이상 시 건폐율/용적률 110% 완화 검토 (현재: {HH}세대)"),
        (22, "교육환경평가서", "가" if is_edu and (T>=100000 or TF>21) else "부", "도정법 28조", f"학교 200m이내 + 10만㎡ 또는 21층 초과, 유치원·설립예정 대안학교 용지 포함 (현재: {T:,}㎡, {TF}층)"),
        ...
        (26, "성능위주 소방설계", "가" if (has_gong and (TF>=50 or H>=200)) or (not (has_gong and len(usages)==1) and (TF>=30 or H>=120)) else "부", "소방시설법", f"아파트 50층/200m 이상 / 일반 30층/120m 이상 (현재: {T:,}㎡, {H}m, {TF}층)"),
        (27, "풍동실험", "판단 유보", "건축구조기준", "협력업체 확인필요"),
        (28, "건축물 교통영향평가", "판단 유보", "도시교통정비법", "50층 이상 대단지 아파트 등 상급관청 단독 심의 격상 대상 여부 확인"),
        (29, "장애물 없는 생활환경(BF) 인증", "가" if is_public or "신축 공공건축물/교통수단·여객시설" in usages or TF>=50 or H>=200 or is_under_link else "부", "이동편의증진법", "신축 공공·여객시설 + 민간 초고층(50층/200m↑) 및 지하연계 복합건축물 의무화"),
        ...
        (41, "소규모 환경영향평가", "판단 유보", "환경영향평가법", "용도지역별 법정 면적 기준 적용 (분할 꼼수 방지 - 인허가 필요 전체 토지 면적 합산 기준)"),
    ...
    if is_daejeon:
        ...
        daejeon_items.append((dj_no_base+1, "건축물 경관심의 [대전]",
            ...
            "대전광역시 경관조례",
            f"21층 이상 또는 10만㎡ 이상 원칙, 서구 등 기초자치단체 조례에 따라 미만도 대상 가능 → 대지 위치 확인 필수 (현재: {TF}층, {T:,}㎡)"))
    ...
    for i in daejeon_items:
        if i[2] == "가":
            ...
            res, color, tag = "✕ 비대상", "#B0B0B0", "non"
            cnts["non"] += 1
        daejeon_display.append({"No": i[0], "분석 항목": i[1], "결과": res, "법적 근거": i[3], "비고": i[4], "color": color, "tag": tag})
    ...
    def make_pdf():
        ...
        pdf.cell(0, 6, f"Copyright © {datetime.now().year} (주)아이팝엔지니어링  |  사업승인 체크리스트 Web v8.0  |  All Rights Reserved.", ln=True, align='C')
        pdf.set_text_color(0, 0, 0)
        ...
        if daejeon_display:
            ...
            pdf.set_font("K", size=11)
            pdf.cell(0, 9, "  ▶ 대전광역시 특화 조례 항목", ln=True, fill=True)
            pdf.set_text_color(0, 0, 0)
    ...
    st.divider()
    pdf_bytes = make_pdf()
    st.download_button("📥 [클릭] 분석 결과 PDF 저장하기", data=bytes(pdf_bytes), file_name=f"Checklist_Boss_v8_{datetime.now().strftime('%H%M%S')}.pdf", mime="application/pdf", type="primary", use_container_width=True)
else:
